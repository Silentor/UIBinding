...
namespace UIBindings.Tests.Runtime
{
    ...
    {
        private IEnumerable<string> _testStringsValue;
        private IEnumerable<int> _testIntsValue;
        ...
        public void BindingByTypeTest_SimplePath( )
        {
            ...
            testBindingByType.CollectionChanged += (o, s) => _testStringsValue = s.Cast<string>().ToArray();
            Assert.That( testBindingByType.IsInited, Is.True );
            testBindingByType.ManuallyCheckChanges();
            Assert.That( _testStringsValue, Is.Null ); //Because source object is null
        ...
        public void BindingByTypeTest_ComplexPath( )
        {
            ...
            //Init without actual source object
            testBindingByType.Init(  );
            testBindingByType.CollectionChanged += (o, i) => _testIntsValue = i.Cast<int>().ToArray();
            Assert.That( testBindingByType.IsInited, Is.True );
            testBindingByType.ManuallyCheckChanges();
            Assert.That( _testIntsValue, Is.Null ); //Because source object is null
